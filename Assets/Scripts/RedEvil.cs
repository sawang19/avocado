...
public class RedEvil : Enemy
{
    ...
    private IEnumerator GolemAttackCo()
    {
            ...
            if ((currentState == EnemyState.idle || currentState == EnemyState.walk) && currentState != EnemyState.stagger && currentState != EnemyState.attack)
            {
                if (findplayer == true)
                {
                    if (Vector3.Distance(target.position, transform.position) <= attackRadius && currentState != EnemyState.stagger)
                    {
                        //agent.velocity = Vector3.zero;
                        //animator.SetBool("Attacking", true);
                        if(Mathf.Abs(target.position.x - transform.position.x) > 0.7 && Mathf.Abs(target.position.y - transform.position.y) > 0.7) 
                        {
                            agent.SetDestination(new Vector3(target.position.x, transform.position.y));
                        }
                        agent.velocity = Vector3.zero;
